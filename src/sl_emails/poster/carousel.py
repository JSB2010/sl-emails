...
from sl_emails.config import POSTER_OUTPUT_HTML
from sl_emails.domain.dates import (
    event_date_for_sort as _event_date_for_sort,
    format_day_long,
    format_poster_week_label,
    resolve_week_bounds,
    time_for_sort as _time_for_sort,
)
from sl_emails.services.event_shapes import (
    PosterEvent,
    fetch_week_events as _fetch_week_events,
    merge_poster_events,
    normalize_custom_event as _normalize_custom_event,
    poster_event_from_dict,
    sort_poster_events,
    source_event_to_poster_event as _source_event_to_poster_event,
)
...
def _load_generate_games_module():
    """Load ingest tooling lazily so tests can still avoid scraper deps until needed."""
    from sl_emails.ingest import generate_games  # noqa: PLC0415
...
def source_event_to_poster_event(source_event: Any, is_varsity_game: Any) -> PosterEvent:
    ...
    return _source_event_to_poster_event(source_event, is_varsity_game)
...
def get_week_bounds(
    ...
    *,
    ...
    return resolve_week_bounds(mode=mode, today=today, start_date=start_date, end_date=end_date)
...
def fetch_week_events(start: date, end: date) -> list[PosterEvent]:
    ...
    return _fetch_week_events(
        start,
        end,
        scrape_athletics_schedule=generate_games.scrape_athletics_schedule,
        fetch_arts_events=generate_games.fetch_arts_events,
...
def render_poster_fragment(model: dict[str, Any], *, poster_id: str = "poster-slide") -> str:
...
def render_poster_fragment_v2(model: dict[str, Any], *, poster_id: str = "poster-slide") -> str:
...
def render_carousel_document(slides: list[dict[str, Any]]) -> str:
    ...
    slides_html = "".join(
        f'<article class="carousel-slide">{render_poster_fragment(slide, poster_id=f"poster-slide-{idx}")}</article>'
...
def _args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-html", type=str, default=str(POSTER_OUTPUT_HTML))
